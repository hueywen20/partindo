...
<div class="report-container">
    ...
    <table class="meta-table">
        ...
        <tr>
            <td colspan="2"><strong>Supplier:</strong> {{ $supplier?->supplier_name ?? 'All Suppliers' }}</td>
        </tr>
    </table>
        ...
        <tbody>
            @forelse($purchases as $index => $purchase)
                <tr>
                    ...
                    <td class="center">{{ \Illuminate\Support\Carbon::parse($purchase->date)->format('d-m-Y') }}</td>
                    <td>{{ $purchase->purchase_inv_no }}</td>
                    <td>{{ $purchase->reference_no ?? '—' }}</td>
                    <td>{{ $purchase->supplier?->supplier_name ?? '—' }}</td>
                    <td class="right">{{ number_format($purchase->grand_total, 2) }}</td>
                    <td class="right">{{ number_format($purchase->final_total, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="center">No purchases found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <table class="summary-table">
        <tr class="total-row">
            <td style="text-align: left;">Total ({{ $purchases->count() }} invoices)</td>
